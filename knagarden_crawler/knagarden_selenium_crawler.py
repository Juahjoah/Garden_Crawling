...
def is_blocked_page(text: str) -> bool:
    ...
    return any(marker in text for marker in text or "")
...
def collect_plant_links(
    ...
    for page in range(1, max_pages + 1):
        ...
        if is_blocked_page(body_text):
            raise BlockedPageError("CloudFront returned a blocked/403 page while collecting links.")
...
def collect_plant_links_with_retries(
    driver: webdriver.Chrome,
    max_pages: int,
    page_delay_min: float,
    page_delay_max: float,
    retries: int,
    blocked_sleep_min: float,
    blocked_sleep_max: float,
) -> list[str]:
    for attempt in range(1, retries + 2):
        try:
            return collect_plant_links(
                driver=driver,
                max_pages=max_pages,
            ...
            sleep_seconds = random.uniform(blocked_sleep_min, blocked_sleep_max)
            print(
                f"[blocked] list pages - wait {sleep_seconds:.0f}s "
...
def extract_plant_detail(driver: webdriver.Chrome, url: str) -> dict:
    ...
    body_text = driver.find_element(By.TAG_NAME, "body").text
    if is_blocked_page(body_text):
        raise BlockedPageError("CloudFront returned a blocked/403 page.")
...
def crawl_detail_with_retries(
    ...
    for attempt in range(1, retries + 2):
        try:
            return extract_plant_detail(driver, url)
        except BlockedPageError as exc:
            if attempt > retries:
                print(f"[blocked] giving up after {attempt} attempts: {url}")
                raise exc

            sleep_seconds = random.uniform(blocked_sleep_min, blocked_sleep_max)
            print(
                f"[blocked] {url} - wait {sleep_seconds:.0f}s "
                f"before retry {attempt}/{retries}"
            )
            time.sleep(sleep_seconds)
...
def main() -> None:
    ...
    parser.add_argument("--blocked-sleep-min", type=float, default=180.0, help="Minimum wait after a 403 page.")
    parser.add_argument("--blocked-sleep-max", type=float, default=600.0, help="Maximum wait after a 403 page.")
